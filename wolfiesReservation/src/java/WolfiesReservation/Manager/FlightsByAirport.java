 ...
 */
public class FlightsByAirport extends HttpServlet {
    ...
    public void service(HttpServletRequest request, HttpServletResponse 
        ...
        try {
            ...
            conn = DriverManager.getConnection(url + dbName, dbUserName,   
                                            dbPassword);
            //Execute Query
            String strQuery = "SELECT DISTINCT F.* FROM Flight F, Leg L, "
                            + "Airport A WHERE F.AirlineID = L.AirlineID AND "
                            + "F.FlightNo = L.FlightNo AND (L.DepAirportId = "
                            + "A.Id OR L.ArrAirportId = A.Id) AND A.Name = ? ";
            PreparedStatement stm = conn.prepareStatement(strQuery);
            stm.setString(1, airportName);
            ...
            String queryType = "All Flights at " + airportName + ": ";
